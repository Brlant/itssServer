<template>
  <div class="app-container">
    ...
    <!-- 资源配置表格 -->
    <div class="whiteBox mmTable" v-show="nowActive == 0">
      <el-table
        :data="tableData"
        class="myTable"
        border
        style="width: 100%"
        max-height="650"
        @row-click="clickResourceRow"
      >
        <el-table-column prop="postName" label="职位" width="120"></el-table-column>
        <el-table-column prop="areaName" label="区域" width="120"></el-table-column>
        <el-table-column prop="gradeName" label="等级" width="120"></el-table-column>
        ...
        <el-table-column fixed="right" label="操作" width="180">
          <template slot-scope="scope">
            <el-button
              ...
              size="small"
              class="priority3"
              @click.native.stop="editResourceRow(scope.row)"
            >
              修改
            </el-button>
            <el-button
              ...
              size="small"
              class="priority1"
              @click.native.stop="delResourceRow(scope.row)"
            >
              删除
            </el-button>
          </template>
...
import {
  ...
  updateChanceConfig,
  addChanceConfig,delChanceConfigLine,addChanceRecord,
  toProject,
  ...
  methods: {
    ...
    editResourceRow(row) {
      getResourceLineDetail(row.id).then((res) => {
        res.data.postTypeActive = true;
        res.data.gradeIdActive = true;
        res.data.nextActive = true;
        ...
        this.getTimeArea([res.data.startTime, res.data.endTime], 0);
      });
