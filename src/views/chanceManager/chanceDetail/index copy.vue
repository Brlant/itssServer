  ...
  <div class="app-container">
    <div class="routerBar">
      ...
      <div class="rightBox">
        ...
        <span class="priority3 ft13" @click="transformProject"
          >| 转为正式项目</span
        >
      </div>
    </div>
    ...
    <div
      class="whiteBox mmTable gaoliang"
      v-show="nowActive == 0"
      style="padding: 1%"
    >
      <el-table
      ...
      >
        ...
        ></el-table-column>
        <el-table-column
          prop="chanceConfigUserList"
          label="最终人选"
          width="120"
        >
          <template slot-scope="scope">
            <span
              v-for="(item, i) in scope.row.chanceConfigUserList"
              class="priority3"
              >{{ item.nickName }}</span
            >
          </template>
      ...
      <div class="resourceLineDetail" v-show="resourceDetailActive">
        ...
        <el-form :model="formData" size="medium" label-width="110px">
          <div
            v-for="(
              chanceConfigItem, chanceConfigIndex
            ) in formData.chanceConfigList"
          >
            <el-row>
              <el-col :span="5">
            ...
            <el-row>
              ...
              <el-col :span="3" class="lineTT">
                计划负荷：<span class="priority3"
                  >{{ chanceConfigItem.planLoad }}%</span
                >
              </el-col>
            ...
            >
              ...
              <el-col :span="3" class="lineTT">
                <div>
                  平均每日工作<span>
                    {{ chanceConfigScheduleItem.workTime }} </span
                  >小时
                </div>
              </el-col>
            </el-row>
              ...
              <el-col :span="5">
                ...
                >
                  <el-select
                    ...
                    :style="{ width: '100%' }"
                    @change="
                      (dates) => editNext('region', dates, chanceConfigIndex)
                    "
                  >
                    <el-option
              ...
              <el-col :span="5">
                ...
                >
                  <el-select
                    ...
                    :style="{ width: '100%' }"
                    @change="
                      (dates) => editNext('postType', dates, chanceConfigIndex)
                    "
                  >
                    <el-option
              ...
              <el-col :span="5">
                ...
                >
                  <el-select
                    ...
                    @change="
                      (dates) =>
                        editNext('postNameId', dates, chanceConfigIndex)
                    "
                  >
                    <el-option
              ...
              <el-col :span="5">
                ...
                >
                  <el-select
                    ...
                    @change="
                      (dates) =>
                        editNext('postLevelId', dates, chanceConfigIndex)
                    "
                  >
                    <el-option
    ...
    <div class="whiteBox mmTable left-lineBox" v-show="nowActive == 1">
      <el-timeline>
        ...
        >
          ...
          <span class="follow follow2"
            >跟进记录：{{ follow.followRecord }}</span
          >
          <span class="follow follow3">跟进方式：{{ follow.followType }}</span>
          <span class="follow follow4"
            >下次跟进时间：{{ follow.nextDate }}</span
          >
        </el-timeline-item>
      </el-timeline>
    </div>
          ...
          <template slot-scope="scope">
            <div>
              ...
              >
                <span
                  class="priority3"
                  @click="addUserToProject(scope.row, scope.$index)"
                  >添加
                </span>
              </el-button>
              <el-button
              ...
              >
                <span
                  class="priority4"
                  @click="delUserToProject(scope.row, scope.index)"
                  >取消
                </span>
              </el-button>
            </div>
      ...
      childDateArea: {
        ...
        disabledDate: (time) => {
          if (this.formData.expectStartTime && this.formData.expectEndTime) {
            ...
            return (
              time.getTime() >
                new Date(this.formData.expectEndTime).getTime() ||
              time.getTime() <
  ...
  methods: {
    ...
    constAll(dates, index) {
      ...
      getTimeProcess(params).then((res) => {
        ...
        this.formData.chanceConfigList[index].workTime = (
          res.data.day * 8
        ).toFixed(2); // 总共多少工时
        if (res.data.day === 0) {
        ...
        this.formData.chanceConfigList[index].chanceConfigScheduleList =
      ...
      switch (who) {
        ...
        case "postLevelId": // 选择职位等级
          ...
          let costNumArry = this.postLevelIdOptions.find((item) => {
            return (
              this.formData.chanceConfigList[index].postLevelId ==
              item.postLevelId
            );
          });
          // 2对外      // 1 对内
          if (costNumArry) {
            if (this.formData.chanceService == 2) {
              this.formData.chanceConfigList[index].costNum =
                costNumArry.costOut;
            } else {
              this.formData.chanceConfigList[index].costNum =
                costNumArry.costIn;
            ...
            console.log(
              " 没有拿到成本 查找出来的数据返回的是undefined ---editNext"
            );
          }
          console.log(
          ...
          // 并计算 下面的周排期
          this.constAll(
            this.formData.chanceConfigList[index].startEndTime,
            index
          );
    ...
    editResourceRow(row, index) {
      ...
      console.log(
        `你好，我是第（${index})条资源配置，我的成本是 +${row.costNum}`
      );
      console.log(row);
      ...
      getRecommendUserList(params).then((res) => {
        res.data.map((item) => {
          ...
          if (
            ...
            _this.formData.chanceConfigList[index].chanceConfigUserList.map(
              (user, u) => {
