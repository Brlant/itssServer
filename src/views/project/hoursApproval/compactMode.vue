<template>
  <div class="compact">
    ...
    <div v-if="childData.list">
      <div
        class="project-list"
        v-for="(item, index) in childData.list"
        :key="index"
      >
        <div class="project">
          <div @click="show(index)">
            ...
            ><i
              ...
              v-if="thisIndex.indexOf(index) != -1"
          ...
          <div>
            <span class="span1"
              >计划 <span class="span2">{{ item.scheduleWorkTimeTotal }}</span
              >小时</span
            >
            <span class="span1"
              >已审 <span class="span2">{{ item.approvedWorkTimeTotal }}</span
              >小时</span
            >
            <span class="span1"
              >待审
              <span class="span2">{{ item.approvalPendingWorkTimeTotal }}</span
              >小时</span
            >
          </div>
        </div>
        <el-table
          :data="item.memberList"
          border
          v-if="thisIndex.indexOf(index) != -1"
          style="width: 100%"
        ...
        >
          <el-table-column
            ...
            :label="i.label"
            :property="i.property"
            :min-width="i.label == '所属部门' ? '250' : '200'"
          >
              ...
              <div class="row">
                ...
                <div class="left" v-if="4 >= index && index > 2">
                  <span
                    v-if="
                      styleRed(
                        matchData(scope.row, 2).content,
                        matchData(scope.row, 3).content,
                        matchData(scope.row, 4).content
                      )
                    "
                    style="color: red"
                  >
                    {{ matchData(scope.row, index).content }}</span
                  >
                ...
                <div class="right" v-if="index > 4">
                  <span
                    style="color: #cfc5c5"
                    v-if="matchData(scope.row, index).approvedWorkTime"
                    >{{ matchData(scope.row, index).approvedWorkTime }}</span
                  >
                  <span
                    v-if="matchData(scope.row, index).approvalPendingWorkTime"
                  >
                    <span
                      v-if="matchData(scope.row, index).approvedWorkTime"
                      style="margin: 0 10px"
                    ...
                    <span style="color: #333">{{
                      matchData(scope.row, index).approvalPendingWorkTime
                    }}</span></span
                  >
                  <span
                    v-if="matchData(scope.row, index).approvalRejectionWorkTime"
                  >
                    <span
                      v-if="
                        matchData(scope.row, index).approvalPendingWorkTime ||
                        (!matchData(scope.row, index).approvalPendingWorkTime &&
                          matchData(scope.row, index).approvedWorkTime)
                      "
                      style="margin: 0 10px"
                      >|</span
                    >
                    <span style="color: red">{{
                      matchData(scope.row, index).approvalRejectionWorkTime
                    }}</span></span
                  >
                </div>
                <div
                  v-if="filterCheck(item.projectId)"
                  v-hasPermi="['project:approval:leader']"
                >
                  <div
                    class="icon"
                    v-if="matchData(scope.row, index).show"
                  ...
                  >
                    <div
                      ...
                      @click="
                        agree(
                          matchData(scope.row, index).workDate,
                          scope.row,
                          item.projectId
                        )
                      "
    ...
    <el-dialog :visible.sync="dialogVisible" width="30%" title="请输入拒绝理由">
      <el-form :model="form">
        <el-form-item>
          <el-input
            v-model="form.reason"
...
export default {
  ...
  methods: {
    ...
    tableTitle() {
      ...
      if (this.childData.workDateList) {
        let tableHead2 = this.childData.workDateList;
        let head = [];
        tableHead2.forEach((item, index) => {
          // console.log(item,index)
          head.push({ label: item, property: `workDate${index}` });
        });
    ...
    //上一周
    pre() {
      this.preDate = moment(this.today, "YYYY/M/D")
        .subtract(7, "days")
        .format("YYYY-MM-DD");
    ...
    //下一周
    next() {
      this.nextDate = moment(this.today, "YYYY/M/D")
        .add(7, "days")
        .format("YYYY-MM-DD");
