    ...
    <div class='icon'>
      ...
      <div style='cursor: pointer;'>
        ...
        <span v-hasPermi="['system:user:add']">
                    <span @click='skillLock(1)' v-if='info.skillLock==0'> 技能锁定 |</span><span @click='skillLock(0)'
                                                                                             v-if='info.skillLock==1'> 技能解锁 |</span>
                </span>

        <span @click='editInfo' v-hasPermi="['system:user:add']"> 编辑 |</span>
        <span v-hasPermi="['system:user:add']"><span @click='stop(1)' v-if='info.status==0'> 停用 |</span><span
          @click='stop(0)' v-if='info.status==1'> 启用 |</span></span>
    ...
    <div>
      ...
      <div>
        ...
        <div style="margin:20px;font-size:18px;">
          ...
          <span class='work' v-for='(item,index) in info.userSkills' :key='index'
                :style="{background: matchColor(item.cssClass)}">{{
              item.skillName
            }}</span>
          <div v-if='info.skillLock==1' style='padding:10px 0px 0px 100px;font-size:12px;color:#a8b5c1;'>
            *改技能锁定后，该成员本人将无法修改技能
          </div>
        </div>
      </div>
    ...
    <div>
      ...
      <div style="font-size:18px;">
        ...
        <div style="margin:20px;font-size:18px;">
          ...
          <span v-if='info.dept' class="content">{{
              info.dept.deptName
            }}</span>
          <span class='title'>负责人:</span>
          <span v-if='info.dept' class="content">{{
              info.dept.leaderName
            }}</span>
        </div>
      </div>
    </div>
