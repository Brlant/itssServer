<template>
  ...
  >
    ...
    >
      <!-- 表单开始 -->
      <el-form-item label="请选择报损日期" prop="scarpTime">
        <el-date-picker
...
import {fileUpload,initiateFrmLoss,getFlow} from "@/api/assetManagement/inventoryManagement";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        scarpTime: [
          { required: true, trigger: 'blur', message: '请选择报损日期' }
        ],
        amount: [
    ...
    submit() {
      this.$refs.elForm.validate(valid => {
        ...
        const data = {
          ...
          inventoryInfoId:this.info.inventoryInfoId
        }
        this.submitLoading = true
        initiateFrmLoss(data).then(res => {
