<template>
  <el-form ref="form" :model="formData" :rules="rules" label-width="120px">
    ...
    <template>
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <el-form-item label="领用人" label-width="80px" prop="recipient" :rules="rules.recipient">
            <el-input v-model="formData.recipient"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="领用部门" label-width="80px" prop="recipientDepartment"
                        :rules="rules.recipientDepartment">
            <el-input v-model="formData.recipientDepartment"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
        ...
        <el-col>
          <el-table :data="formData.details" border>
            ...
            <el-table-column prop="price" label="含税进价">
              <template slot-scope="scope">
                <el-input :readonly="true" :disabled="true" v-model.number="scope.row.price"
                          placeholder="请输入含税进价" style="width: 100%"></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="taxRate" label="税率">
              <template slot-scope="scope">
                <el-input :readonly="true" :disabled="true" v-model.number="scope.row.taxRate"
                          placeholder="请输入税率" style="width: 100%"></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="priceWithoutTax" label="不含税进价">
              <template slot-scope="scope">
                <el-input :readonly="true" :disabled="true" v-model.number="scope.row.priceWithoutTax"
            ...
            <el-table-column prop="sellingPriceIncludingTax" label="含税售价">
              <template slot-scope="scope">
                <el-form-item :prop="`details.${scope.$index}.sellingPriceIncludingTax`"
                              :rules="rules.sellingPriceIncludingTax">
                  <el-input :rules="rules.sellingPriceIncludingTax"
            ...
            <el-table-column prop="taxRateIncluding" label="税率">
              <template slot-scope="scope">
                <el-form-item :prop="`details.${scope.$index}.taxRateIncluding`" :rules="rules.taxRateIncluding">
                  <el-input :rules="rules.taxRateIncluding"
            ...
            <el-table-column prop="grossMargin" label="毛利率">
              <template slot-scope="scope">
                <span>{{
                    (scope.row.sellingPriceIncludingTax - scope.row.price) / scope.row.sellingPriceIncludingTax
                  }}</span>
              </template>
            </el-table-column>
            <el-table-column label="操作">
              <template slot-scope="scope">
                <el-button type="primary" v-if="scope.$index === 0" size="mini" circle
  ...
  data() {
    return {
      ...
      rules: {
        ...
        recipientDepartment: [{required: true, message: '请输入领用部门', trigger: 'blur'}],
